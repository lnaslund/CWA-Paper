...
stream.no3<-merge(difference_no3,loading.alt,by="state") %>% filter(is.na(WLR_Trend_Error)==FALSE)
stream.nh4<-merge(difference_nh4,loading.alt,by="state") %>% filter(is.na(WLR_Trend_Error)==FALSE)
stream.tp<-merge(difference_tp,loading.alt,by="state") %>% filter(is.na(WLR_Trend_Error)==FALSE)
stream.tn<-merge(difference_tn,loading.alt,by="state") %>% filter(is.na(WLR_Trend_Error)==FALSE)
lake.tp<-merge(difference_tp_lake,loading.alt,by="state") %>% filter(is.na(WLR_Trend_Error)==FALSE)
lake.tn<-merge(difference_tn_lake,loading.alt,by="state") %>% filter(is.na(WLR_Trend_Error)==FALSE)

#### Stream NO3
stream.no3.global<-lm(WLR_Trend_Estimate~pop+pop.delta+urban+undeveloped+ag+urban.delta+undeveloped.delta+ag.delta+
                        fertilizer+fertilizer.delta+feed+feed.delta,stream.no3,na.action = "na.fail")
best<-dredge(stream.no3.global)
subset(best,delta<2)
best.stream.no3<-lm(WLR_Trend_Estimate~feed,data=stream.no3)

### Stream NH4
stream.nh4.global<-lm(WLR_Trend_Estimate~pop+pop.delta+urban+undeveloped+ag+urban.delta+undeveloped.delta+ag.delta+
                        fertilizer+fertilizer.delta+feed+feed.delta,stream.nh4,na.action = "na.fail")
best<-dredge(stream.nh4.global)
subset(best,delta<2)
best.stream.nh4<-lm(WLR_Trend_Estimate~feed+urban.delta,data=stream.nh4)

### stream TP
stream.tp.global<-lm(WLR_Trend_Estimate~pop+pop.delta+urban+undeveloped+ag+urban.delta+undeveloped.delta+ag.delta+
                       fertilizer+fertilizer.delta+feed+feed.delta,stream.tp,na.action = "na.fail")
best<-dredge(stream.tp.global)
subset(best,delta<2)
best.stream.tp<-lm(WLR_Trend_Estimate~ag.delta,data=stream.tp)

### stream TN
stream.tn.global<-lm(WLR_Trend_Estimate~pop+pop.delta+urban+undeveloped+ag+urban.delta+undeveloped.delta+ag.delta+
                       fertilizer+fertilizer.delta+feed+feed.delta,stream.tn,na.action = "na.fail")
best<-dredge(stream.tn.global)
subset(best,delta<2)
best.stream.tn<-lm(WLR_Trend_Estimate~feed+feed.delta,data=stream.tn)
summary(best.stream.tn)

### lake TP
lake.tp.global<-lm(WLR_Trend_Estimate~pop+pop.delta+urban+undeveloped+ag+urban.delta+undeveloped.delta+ag.delta+
                     fertilizer+fertilizer.delta+feed+feed.delta,lake.tp,na.action = "na.fail")
best<-dredge(lake.tp.global)
subset(best,delta<2)
best.lake.tp<-lm(WLR_Trend_Estimate~ag+fertilizer.delta+undeveloped,data=lake.tp)
summary(best.lake.tp)

### lake TN
lake.tn.global<-lm(WLR_Trend_Estimate~pop+pop.delta+urban+undeveloped+ag+urban.delta+undeveloped.delta+ag.delta+
                     fertilizer+fertilizer.delta+feed+feed.delta,lake.tn,na.action = "na.fail")

best<-dredge(lake.tn.global)
subset(best,delta<2)
best.lake.tn<-lm(WLR_Trend_Estimate~ag+feed.delta+fertilizer.delta+pop.delta,lake.tn)
...
for (i in 1:nsims){
  ...
  d$slope.wlr<-rnorm(nrow(d),d$WLR_Trend_Estimate,d$WLR_Trend_Error)
...
for (i in 1:nsims){
  ...
  model.money.wlr<-lm(slope.wlr~ag+feed.delta+fertilizer.delta+pop.delta+total.money.sq.km,data=d)
  model.money.slr<-lm(slope.slr~ag+feed.delta+fertilizer.delta+pop.delta+total.money.sq.km,data=d)
  model.money.wlr.z<-lm(slope.wlr~ag+feed.delta+fertilizer.delta+pop.delta+money.z,data=d)
  model.money.slr.z<-lm(slope.slr~ag+feed.delta+fertilizer.delta+pop.delta+money.z,data=d)
  
  model.criteria.wlr<-lm(slope.wlr~ag+feed.delta+fertilizer.delta+pop.delta+stream.n.criteria,data=d)
  model.criteria.slr<-lm(slope.slr~ag+feed.delta+fertilizer.delta+pop.delta+stream.n.criteria,data=d)
  model.criteria.wlr.z<-lm(slope.wlr~ag+feed.delta+fertilizer.delta+pop.delta+stream.n.criteria.z,data=d)
  model.criteria.slr.z<-lm(slope.slr~ag+feed.delta+fertilizer.delta+pop.delta+stream.n.criteria.z,data=d)
  
  model.tmdl.wlr<-lm(slope.wlr~ag+feed.delta+fertilizer.delta+pop.delta+tmdl.sites.sq.km,data=d)
  model.tmdl.slr<-lm(slope.slr~ag+feed.delta+fertilizer.delta+pop.delta+tmdl.sites.sq.km,data=d)
  model.tmdl.wlr.z<-lm(slope.wlr~ag+feed.delta+fertilizer.delta+pop.delta+tmdl.z,data=d)
  model.tmdl.slr.z<-lm(slope.slr~ag+feed.delta+fertilizer.delta+pop.delta+tmdl.z,data=d)
  
  null.wlr<-lm(slope.wlr~ag+feed.delta+fertilizer.delta+pop.delta,data=d)
  null.slr<-lm(slope.slr~ag+feed.delta+fertilizer.delta+pop.delta,data=d)
  ...
  z.scores.models.wlr<-data.frame(model.type="WLR.z",policy=c("tmdl","319","criteria"),
                                  coef=c(coef(model.tmdl.wlr.z)[6],
                                         coef(model.money.wlr.z)[6],
                                         coef(model.criteria.wlr.z)[6]),
                                  coef.se=c(wlr.z.tmdl.summary$coefficients[6,2],
                                            wlr.z.money.summary$coefficients[6,2],
                                            wlr.z.criteria.summary$coefficients[6,2]))
  ...
  z.scores.models.slr<-data.frame(model.type="SLR.z",policy=c("tmdl","319","criteria"),
                                  coef=c(coef(model.tmdl.slr.z)[6],
                                         coef(model.money.slr.z)[6],
                                         coef(model.criteria.slr.z)[6]),
                                  coef.se=c(slr.z.tmdl.summary$coefficients[6,2],
                                            slr.z.money.summary$coefficients[6,2],
                                            slr.z.criteria.summary$coefficients[6,2]))
  ...
  output.wlr<-data.frame(model.type="WLR",policy=c("tmdl","319","criteria"),
                         coef=c(coef(model.tmdl.wlr)[6],
                                coef(model.money.wlr)[6],
                                coef(model.criteria.wlr)[6]),
                         coef.se=c(wlr.tmdl.summary$coefficients[6,2],
                                   wlr.money.summary$coefficients[6,2],
                                   wlr.criteria.summary$coefficients[6,2]))
  ...
  output.slr<-data.frame(model.type="SLR",policy=c("tmdl","319","criteria"),
                         coef=c(coef(model.tmdl.slr)[6],
                                coef(model.money.slr)[6],
                                coef(model.criteria.slr)[6]),
                         coef.se=c(slr.tmdl.summary$coefficients[6,2],
                                   slr.money.summary$coefficients[6,2],
                                   slr.criteria.summary$coefficients[6,2]))
...
write.csv(z_scores_plot_melt,"./data/clean_data/z_scores_estimates.csv")
...
non_z_summary_new<-data.frame(policy=non_z_summary$policy,nutrient_type=non_z_summary$nutrient_type,new_value=rnorm(nrow(non_z_summary),non_z_summary$coef,non_z_summary$coef.se))
non_z_melt<-melt(non_z_summary_new)

write.csv(non_z_melt,"./data/clean_data/non_z_scored_estimates.csv")
